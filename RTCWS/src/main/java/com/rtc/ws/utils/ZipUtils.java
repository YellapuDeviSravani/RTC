...
public class ZipUtils {
	...
	public void generateFileList(File node,String folder) {
		// add file only
		if (node.isFile() && !node.getName().startsWith(".")) {
			if( folder == null || folder.isEmpty() || folder.equals(node.getParentFile().getName())){
		...
		if (node.isDirectory()) {
			...
			for (String filename: subNote) {
				...
				generateFileList(new File(node, filename),folder);
