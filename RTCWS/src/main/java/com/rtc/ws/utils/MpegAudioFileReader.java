...
/**
 * This class implements AudioFileReader for MP3 SPI. The below implementation is taken from
 * javazoom.spi.mpeg.sampled.file.MpegAudioFileReader and has been modified to fix the issue while
 * reading input streams for MP3 files. 
 * 
 * The changes marked with RTC_CHANGES are made in the original version
        ...
        try
        {
            ...
            if (mLength != AudioSystem.NOT_SPECIFIED)
            {
               ...
               // aff_properties.put("duration", new Long((long) nTotalMS * 1000L));
                aff_properties.put("duration", new Long((long) nTotalMS));
            }
